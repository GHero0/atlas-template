  ...
  box(width: 0.7cm, height: 0.7cm)[
    ...
    #box(width: 2 * orbit-radius + 2 * planet-radius, height: 2 * orbit-radius + 2 * planet-radius)[
      ...
      #place(
        ...
        circle(radius: orbit-radius, fill: none, stroke: stroke-width, circle(fill: black, radius: 5pt)),
      )
      #place(
        center + horizon,
        dx: cx, // Now these offsets are relative to the center
        dy: cy,
        circle(radius: planet-radius, fill: black),
      )
    ...
    #for row in range(grid-range) {
      for col in range(grid-range) {
        ...
        if distance <= (big-radius / 1pt) {
          ...
          place(dx: x - radius, dy: y - radius, circle(radius: radius, fill: black))
        }
      }
    }
  ]
...
// ATLAS-logos
#let ATLAS-logo-text(width: auto, height : auto) = image("assets/ATLAS-logo/ATLAS-logo-text.svg", width: width, height: height)
#let ATLAS-logo-full(width: auto, height : auto) = image("assets/ATLAS-logo/ATLAS-logo-full.svg", width: width, height: height)
#let ATLAS-logo-icon(width: auto, height : auto) = image("assets/ATLAS-logo/ATLAS-logo-icon.svg", width: width, height: height)
...
#let ATLAS-document(
  ...
  set text(font: ATLAS_font, lang: "en")
  ...
  show figure: body => {
    rect(inset: 10pt, radius: 10pt, stroke: ATLAS_black.transparentize(90%), body)
  }
